...
`policy_cache_store` fixture): `PolicyCache` rides `migrated_db` and skips
cleanly when Postgres is unreachable; `InMemoryPolicyCache` — spec §11's
dry-run cache — needs no database at all, which is its whole point. Running
the same tests over both is what proves a dry-run's quarantine verdict is the
one production would give the same body. Cases only the real store can express
(SQL row counts, transaction timestamps, `list_for_tenant`, the CHECK
constraints, the logged refusal) stay DB-only below.
"""
...
def policy_cache_store(request):
    """One policy cache per param: the real Postgres-backed store (riding
    `migrated_db`, so the whole param skips cleanly when Postgres is
    unreachable) or the in-memory dry-run cache. The shared contract tests
    take this fixture so both implementations classify and guard with the
    same code — a dry-run's verdict is provably the one production would
    give the same body."""
...
def test_unknown_version_reads_as_none(policy_cache_store):
    # A never-fetched version is a cache MISS, not an error and not an empty
    # policy set.
    assert policy_cache_store.get("gv-never-fetched") is None


def test_a_valid_version_round_trips(policy_cache_store):
    policy_cache_store.put(_resolved(VERSION_ID, VALID_BODY))
    row = policy_cache_store.get(VERSION_ID)
...
def test_the_body_is_stored_verbatim(policy_cache_store):
...
def test_a_cached_body_re_parses_to_the_policy_set(policy_cache_store):
...
def test_a_quarantined_version_persists_with_its_reason(policy_cache_store):
    ...
    policy_cache_store.put(_resolved("gv-prose", PROSE_BODY))
    row = policy_cache_store.get("gv-prose")
    assert row is not None
...
def test_a_quarantined_body_re_parses_to_the_malformed_result(policy_cache_store):
    policy_cache_store.put(_resolved("gv-prose", PROSE_BODY))
    row = policy_cache_store.get("gv-prose")
    assert row is not None
    parsed = row.parse()
...
def test_re_classification_clears_a_stale_quarantine_reason(policy_cache_store):
    ...
    policy_cache_store.put(_resolved(VERSION_ID, PROSE_BODY))
    policy_cache_store.put(_resolved(VERSION_ID, VALID_BODY))
    row = policy_cache_store.get(VERSION_ID)
    assert row is not None
    assert row.outcome is ParseOutcome.valid
...
def test_versions_do_not_share_a_row(policy_cache_store):
    # The ledger references old version ids forever, so a superseded version
    # must stay readable alongside its successor.
    policy_cache_store.put(_resolved("gv-0001", VALID_BODY))
    policy_cache_store.put(_resolved("gv-0002", PROSE_BODY))
    first = policy_cache_store.get("gv-0001")
    second = policy_cache_store.get("gv-0002")
...
def test_put_returns_the_classification(policy_cache_store):
    ...
    valid = policy_cache_store.put(_resolved(VERSION_ID, VALID_BODY))
    assert isinstance(valid, PolicySet)
    quarantined = policy_cache_store.put(_resolved("gv-prose", PROSE_BODY))
    assert isinstance(quarantined, MalformedPolicySet)


def test_a_tenant_mismatched_body_quarantines(policy_cache_store):
    ...
    parsed = policy_cache_store.put(
        _resolved("gv-cross", VALID_BODY, tenant="snowlinedev")
    )
    assert isinstance(parsed, MalformedPolicySet)
    assert parsed.reason is MalformedPolicyReason.tenant_mismatch
    row = policy_cache_store.get("gv-cross")
    assert row is not None
...
def test_a_cross_tenant_id_collision_is_refused(policy_cache_store):
    ...
    policy_cache_store.put(_resolved("pv-0001", VALID_BODY, tenant=TENANT))
    other = json.loads(VALID_BODY)
    other["tenant"] = "snowlinedev"
    other_body = json.dumps(other)
    refused = policy_cache_store.put(
        _resolved("pv-0001", other_body, tenant="snowlinedev")
    )
    ...
    row = policy_cache_store.get("pv-0001")
    assert row is not None
    ...
    resolution = resolve_policy_set(
        "snowlinedev", provider=provider, cache=policy_cache_store
    )
...
# --- DB-only: SQL row counts, transaction timestamps, listings, CHECKs --------
...
def test_a_row_survives_the_instance_that_wrote_it(migrated_db):
...
def test_a_mismatched_version_is_listed_under_the_requesting_tenant(migrated_db):
    # The listing half of `test_a_tenant_mismatched_body_quarantines`: the
    # quarantined row shows up in the REQUESTING tenant's §11 listing, which
    # is the operator surface where the misregistration gets noticed.
    cache = PolicyCache()
    cache.put(_resolved("gv-cross", VALID_BODY, tenant="snowlinedev"))
